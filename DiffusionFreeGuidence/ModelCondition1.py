...
class TimeEmbedding(nn.Module):
    def __init__(self, T, d_model, dim):  # T=1000 d_model  d_model=32  dim=32*4 把t映射到128维向量上。
        assert d_model % 2 == 0
        super().__init__()
...
class ConditionalEmbedding3(nn.Module):
    def __init__(self, d_model, dim):
        assert d_model % 2 == 0
        super().__init__()
        self.condEmbedding2 = nn.Sequential(
...
class ConditionalEmbedding4(nn.Module):
    def __init__(self, d_model, dim):
        assert d_model % 2 == 0
        super().__init__()
        self.condEmbedding2 = nn.Sequential(
...
class DownSample(nn.Module):
    ...
    def forward(self, x, temb, cemb1, cemb2, cemb3, cemb4):
...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, tdim, dropout, attn=False):
        ...
        self.cond_proj3 = nn.Sequential(
            Swish(),
            nn.Linear(tdim, out_ch),
        )
        self.cond_proj4 = nn.Sequential(
            Swish(),
            nn.Linear(tdim, out_ch),
        )
    ...
    def forward(self, x, temb, cemb1, cemb2, cemb3, cemb4):
        ...
        # 处理所有条件嵌入
        h += self.cond_proj1(cemb1)[:, :, None, None, None]
        h += self.cond_proj2(cemb2)[:, :, None, None, None]
        h += self.cond_proj3(cemb3)[:, :, None, None, None]
        h += self.cond_proj4(cemb4)[:, :, None, None, None]
        h = self.block2(h)
...
class UNet(nn.Module):
    def __init__(self, T, ch, ch_mult, num_res_blocks, dropout):
        ...
        # 喉道半径均值和方差的拼接嵌入
        self.cond_embedding3 = ConditionalEmbedding3(ch * 2, tdim)
        # 配位数均值和方差的拼接嵌入
        self.cond_embedding4 = ConditionalEmbedding4(ch * 2, tdim)
        ...
        now_ch = ch
        for i, mult in enumerate(ch_mult):  # [0123][1224]
            out_ch = ch * mult
        ...
        for i, mult in reversed(list(enumerate(ch_mult))):  # 1 2 2 4
            out_ch = ch * mult
    ...
    def forward(self, x, t, labels, label2, label3, label4):
        # Timestep embedding
        temb = self.time_embedding(t)
        # cemb = self.cond_embedding1(labels)
        cemb1 = self.cond_embedding1(labels).to('cuda')
        cemb2 = self.cond_embedding2(label2).to('cuda')
        cemb3 = self.cond_embedding3(label3).to('cuda')
        cemb4 = self.cond_embedding4(label4).to('cuda')
        ...
        for layer in self.downblocks:
            # 传递所有条件嵌入（cemb1, cemb2, cemb3, cemb4）
            h = layer(h, temb, cemb1, cemb2, cemb3, cemb4)
            hs.append(h)
        # Middle
